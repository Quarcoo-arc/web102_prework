...
const numGames = GAMES_JSON.reduce((sum, game) => sum + 1, 0);

gamesCard.innerHTML = `<p>${numGames.toLocaleString()}</p>`;
...
function filterUnfundedOnly() {
  ...
  const notMetGoal = GAMES_JSON.filter((game) => game.goal > game.pledged);
...
function filterFundedOnly() {
  ...
  const metOrExceededGoal = GAMES_JSON.filter(
    (game) => game.goal <= game.pledged
  );

  // use the function we previously created to add unfunded games to the DOM
  addGamesToPage(metOrExceededGoal);
}
...
const unfundedGamesCount = GAMES_JSON.reduce(
  (sum, game) => (game.pledged < game.goal ? sum + 1 : sum + 0),
  0
);

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${totalContributions.toLocaleString()} has been raised for ${numGames} games. Currently, ${unfundedGamesCount} ${
  unfundedGamesCount == 1 ? "games remain" : "game remains"
